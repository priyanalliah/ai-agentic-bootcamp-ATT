...
- [Use case: Supervisor Assistant](#use-case-noc-supervisor-assistant)  
  - [Table of Contents- **Purpose*##### S- **Usage- *- **Usage- **Purpose**: Verifies whether a specific server or URL is currently onlin         - `Th      ```
      - Answer questions about the operational status of Dish's network based on the provided site and node data.This includes information about nodes, incidents, and overall health of regions or specific locations.Network Status Agent specializes i#### 3) Quick sanity checks
- Ask: "Check if dish.com is - "**Check if dish.com is up.**" → should route to **Server Status Agent** (calls `check_server_status`)p."  
- The agent should call the `check_server_status` tool and return whether the server is reachable.nswering inquiries about the current operational status of Dish's network. It has access to up-to-date data about nodes sites, and services—such as cell towers, routers, and backhaul links—summarizes ongoing incidents, and provides a concise overview to the user.` and reachable.  
- **Tools**: Uses a server check tool to confirm availability.  
- **Usage**: Handles requests like "Check if dish.com is up."  
 

##### Supervisor Agenten the Supervisor requests an incident update for the "Los Angeles Dish Network team," this agent generates the email body and sends it through Outlook.Purpose**: Verifies whether a specific server or URL is currently onlin         - `Th      ```
      - Answer questions about the operational status of Dish's network based on the provided site and node data.This includes information about nodes, incidents, and overall health of regions or specific locations.Network Status Agent specializes i#### 3) Quick sanity checks
- Ask: "Check if dish.com iTry these natural-language prompts to validate routing:
...
##### Supervisor Agenten the Supervisor requests an incident update for the "Los Angeles Dish Network team," this agent generates the email body and sends it through Outlook.rver Status Agent  
- **Purpose**: Verifies whether a specific s     4. Add a des     1. Scroll down to the **Behavior** section and add the following instructions.
      ```
      - Answer questions about the operational status of Dish's network based on the provided site and node data.This includes information about nodes, incidents, and ov#### 3) Quick sanity checks
- Ask: "Check if dish.com iTry these natural-language prompts to validate routing:
- "**What's the status of site S002?**" → should route to **Network Status Agent** (calls `get_data`)
- "**Check if dish.com is up.**" → should route to **Server Status Agent** (calls `check_server_status`)
- "**Here's an incident log… what's the root cause and fix?**" → should route to **Incident Diagnosis Agent** (uses `diagnose_incident_log`, consults resolution guides if configured)
- "**Draft an email to the LA network team that the incident is resolved.**" → should route to **Communications Agent** (and can send via Outlook if configured)"  
- The agent should call the `check_server_status` tool and return whether the server is reachable.ll health of regions or specific locations. 
      ...
      ```ion for your agent. 
         ...
         - `The Network Status Agent specializes in answering inquiries about the current operational status of Dish's network. It has access to up-to-date data about nodes sites, and services—such as cell towers, routers, and backhaul links—summarizes ongoing incidents, and provides a concise overview to the user.`er or URL is currently online and reachable.  
- **Tools**: Uses a server check tool to confirm availability.  
- **Usage**: Handles requests like "Check if dish.com is up."rafts professional and concise email updates for internal or external stakeholders about incidents or operational changes.  
...
- **Usage**: When the Supervisor requests an incident update for the "Los Angeles Dish Network team," this agent generates the email body and sends it through Outlook.table-of-contents)  
